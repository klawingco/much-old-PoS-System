...
namespace coop_main
{
    public partial class saved_sales : Form
    {
        DatabaseStuff dbhandler = new DatabaseStuff();
        String empID;
        public saved_sales(String empId)
        {
            InitializeComponent();
            this.empID = empId;
        }
        ...
        private void saved_sales_Load(object sender, EventArgs e)
        {
          ...
          //  {
           ...
           // }
            dbhandler.populateSavedTrans(dataGridView1,"1");
        }

        private void btn_openReg_Click(object sender, EventArgs e)
        {
            open_register reg = new open_register(empID);
            reg.Show();
            this.Close();
        }
        void www() {
            ...
            open_register reg = new open_register(empID, id, dt, row.Cells[2].Value.ToString(), row.Cells[1].Value.ToString());
            reg.Show();
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            if(dataGridView1.Rows.Count > 0){
                ...
                DataGridViewRow row = dataGridView1.CurrentCell.OwningRow;
                String id = row.Cells[0].Value.ToString();
                dbhandler.deleteSavedSales(id);
                dbhandler.populateSavedTrans(dataGridView1,"1");
            }
            
        }

        private void btn_openInReg_Click(object sender, EventArgs e)
        {
            if(dataGridView1.Rows.Count > 0){
                www();
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
           // MessageBox.Show(dateTimePicker1.Value.ToString());
            ...
            if(ch_startDate.Checked && ch_endDate.Checked){
                dbhandler.populateSavedTransBetweem(dataGridView1, dateTimePicker1.Value, dateTimePicker2.Value);

            }
            else if (ch_startDate.Checked && !ch_endDate.Checked)
            {
                dbhandler.populateSavedTransBetweem(dataGridView1, dateTimePicker1.Value, dateTimePicker1.Value);

            }
            else if (!ch_startDate.Checked && ch_endDate.Checked)
            {
                dbhandler.populateSavedTransBetweem(dataGridView1, dateTimePicker2.Value, dateTimePicker2.Value);
        ...
        {
            var a = DateTime.Now;
            label6.Text = "Today is " + a.ToString("MM-dd-yyyy") + " " + a.ToLongTimeString();
        }
